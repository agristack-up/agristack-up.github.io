import os
import re

# Function to format game name for content replacement
def format_game_name(game_name):
    return game_name.replace("-", " ").title()  # Replace hyphen with space & capitalize words

# Game titles list
games = [
    "10-minutes-till-dawn",
"11-11",
"12-mini-battles-2",
"12-minibattles",
"18-wheeler-cargo-simulator",
"1v1-lol-offline",
"1v1-lol",
"2-minute-football",
"2048-multitask",
"2048",
"3d-car-simulator",
"3d-monster-truck-skyroads",
"3d-moto-simulator-2",
"4th-and-goal-2022",
"4x4-drive-offroad",
"8-ball-pool",
"8bit-fiesta",
"9-balls-pool",
"a-dance-of-fire-and-ice",
"a-pretty-odd-bunny-roast-it",
"a-pretty-odd-bunny",
"a-small-world-cup",
"air-hockey-championship-deluxe",
"aliens-nest",
"amazing-bubble-breaker",
"amazing-bubble-connect",
"among-us",
"animal-arena",
"ape-sling",
"aqua-thrills",
"archer-master-3d-castle-defense",
"archery-world-tour",
"arithmetica",
"arrow-pathway",
"athletics-hero",
"australian-patience",
"avalanche",
"b-cubed",
"bacon-may-die",
"bad-ice-cream-2",
"bad-ice-cream-3",
"barbershop-inc",
"basket-and-ball",
"basket-bros",
"basket-champs",
"basket-random",
"basket-swooshes",
"basketball-frvr",
"basketball-legends",
"basketball-stars",
"battle-golf",
"battle-wheels",
"bearsus",
"become-a-dentist",
"become-a-fashion-designer",
"become-a-puppy-groomer",
"big-shot-boxing",
"big-tall-small",
"bike-trials-offroad-1",
"bike-trials-winter-1",
"bike-trials-winter-2",
"bird-jumper",
"bitlife",
"blob-drop",
"blobby-clicker",
"block-the-pig",
"blockins",
"blocky-cars",
"blocky-trials",
"blood-drift",
"bloons-tower-defense-1",
"blue",
"blumgi-ball",
"blumgi-bloom",
"blumgi-castle",
"blumgi-dragon",
"blumgi-rocket",
"blumgi-slime",
"bob-the-robber-4",
"bomber-royale",
"bottle-flip",
"bouncy-basketball",
"bowling-stars",
"box-kid-puzzles",
"boxing-fighter-shadow-battle",
"boxing-physics-2",
"boxing-random",
"boxrob-2",
"boxrob-3",
"boxrob",
"brain-dozer",
"brain-test-2-tricky-stories",
"brain-test-3-tricky-quests",
"brain-test-4-tricky-friends",
"brain-test-tricky-puzzles",
"breaking-the-bank",
"bubble-shooter",
"bubble-trouble-3",
"bubble-trouble",
"bubbles-cool",
"bullet-bro",
"bullet-force",
"bumper-cars-soccer",
"bumpy-jumpy",
"bunny-hop",
"burger-bounty",
"burnin-rubber-5-xs",
"burnin-rubber-crash-n-burn",
"burnout-drift-seaport-max",
"burnout-extreme-drift-2",
"burrito-bison",
"candy-jump",
"car-climb-racing",
"car-drift-racers-2",
"car-eats-car-winter-adventure",
"car-rush",
"car-simulator-arena",
"cars-thief-tank-edition",
"cars-thief",
"castle-of-chaos",
"casual-chess",
"cat-coffee-shop",
"cat-gunner-super-zombie-shoot",
"cat-room-blast",
"cat-trap",
"catch-the-candy-html5",
"catpad",
"cats-love-cake-2",
"cats-love-cake",
"cats",
"cave-blast",
"chess-grandmaster",
"chessformer",
"chicken-merge",
"chicken-sword-ninja-master",
"chicky-farm",
"circloo-2",
"city-bike-stunt-2",
"city-car-driving-stunt-master",
"city-rider",
"clash-of-tanks",
"classic-mahjong",
"cluster-rush",
"coffee-shop",
"color-switch",
"color-tunnel-2",
"colorup",
"combo-crusader",
"connect-248",
"cookie-clicker",
"cooking-fast-halloween",
"cooking-fast",
"cosmos-lines",
"court-kings-3d",
"cover-orange-journey",
"cover-orange-wild-west",
"cow-bay",
"cozy-room-design",
"craftomation-1",
"crazy-tunnel-3d",
"cricket-world-cup",
"crossy-road",
"crown-guard",
"cubes-king",
"cubies",
"cubinko",
"cubito-mayhem",
"curve-ball-3d",
"cute-puzzle-witch",
"cyber-cars-punk-racing",
"daddy-long-legs",
"dadish-3d",
"dadish",
"dana-relics-for-sale",
"darts-pro",
"death-chase",
"death-run-3d",
"deepest-sword",
"deer-simulator",
"demolition-derby-crash-racing",
"descent",
"detective-dan",
"detective-loupe-puzzle",
"digging-master",
"dino-bros",
"dinosaur-game",
"doctor-acorn-3",
"doctor-hero",
"doctor-teeth-2",
"doctor-teeth",
"dog-simulator-3d",
"doge-miner",
"doodle-champion-island",
"doodle-jump",
"double-panda",
"dragon-simulator-3d",
"draw-pixel-art",
"draw-the-hill",
"dreadhead-parkour",
"dress-up-the-lovely-princess",
"drift-boss",
"drift-hunters",
"driftwave",
"drive-mad",
"drunken-duel",
"dubai-police-parking-2",
"duck-life-2",
"duck-life-3-evolution",
"duck-life-4",
"duck-life-battle",
"duck-life",
"duck-pond-mahjong",
"ducklingsio",
"dungeons-dressups",
"dunk-perfect",
"dunkbrush",
"dunkers",
"duo-survival-2",
"duo-survival-3",
"duo-survival",
"duo-vikings-2",
"duo-vikings-3",
"duo-vikings",
"dyna-boy",
"earn-to-die",
"eggbot-vs-zombies",
"eggy-car",
"elastic-man",
"electron-dash",
"element-bblocks",
"eliza-mall-mania",
"escaping-the-prison",
"eugenes-life",
"evo-city-driving",
"extreme-car-driving-simulator",
"extreme-off-road-cars-3-cargo",
"extreme-off-road-cars",
"factory-balls-forever",
"fairy-dressup",
"fancy-pants-2",
"fancy-pants-3",
"fancy-pants",
"farm-battles",
"fast-food-dumpster-adventure",
"final-ninja-html5",
"finger-cook",
"fireboy-and-watergirl-1-forest-temple",
"fireboy-and-watergirl-2-light-temple",
"fireboy-and-watergirl-3-ice-temple",
"fireboy-and-watergirl-4-crystal-temple",
"fireboy-and-watergirl-5-elements",
"fireboy-and-watergirl-6-fairy-tales",
"flags-maniac",
"flags-quiz",
"flappy-bird-origin",
"flappy-bird",
"fleeing-the-complex",
"flip-bros",
"flip-runner",
"foot-chinko",
"football-blitz",
"football-penalty-champions",
"footyzag",
"forgotten-hill-memento-love-beyond",
"forgotten-hill-the-wardrobe-3",
"forgotten-hill-the-wardrobe-4",
"fortride-open-world",
"fortz",
"four-in-a-row",
"fox-simulator-3d",
"free-kick-shooter",
"free-the-key",
"freefall-apart",
"freeway-fury",
"friday-night-funkin-b-sides",
"friday-night-funkin-vs-hatsune-miku",
"friday-night-funkin-vs-shaggy-x-matt",
"friday-night-funkin-vs-xe",
"fruit-ninja",
"fun-water-sorting",
"funny-dentist-surgery",
"funny-ear-surgery",
"funny-eye-surgery",
"funny-shooter-2",
"funny-throat-surgery",
"furious-racing-3d",
"fury-wars",
"g-switch-3",
"g-switch-4",
"game-of-farmers",
"gangster-contract-mafia-wars",
"gem-11",
"genie-quest",
"geometry-dash",
"getaway-shootout",
"gladiator-true-story",
"glam-girl-dress-up-and-makeover",
"go-kart-go-ultra",
"goat-vs-zombies",
"gobble",
"gobdun",
"gold-digger-frvr",
"golf-champions",
"golfinity",
"google-feud",
"goose-game",
"grand-prix-hero",
"grindcraft",
"grow-up-the-cats",
"growmi",
"gun-mayhem-2",
"gun-mayhem-3",
"gun-mayhem",
"gunspin",
"hammer-2-reloaded",
"hangman",
"head-soccer-2023",
"heads-arena-soccer-all-stars",
"heart-star",
"hide-and-smash",
"highway-bike-simulator",
"highway-racer-3d",
"highway-rider-extreme",
"highway-traffic",
"hills-of-steel",
"hipster-vs-rockers",
"hop-chop",
"hop-warp",
"hoppenhelm",
"horse-shoeing",
"horse-simulator-3d",
"house-of-hazards",
"hover-racer-drive",
"hover-racer",
"hypersnake",
"ice-cream-please",
"icy-purple-head-3",
"idle-ants",
"idle-digging-tycoon",
"idle-gold-miner",
"idle-lumber-inc",
"idle-miner",
"idle-mining-empire",
"idle-startup-tycoon",
"idle-zoo-safari-rescue",
"iidle-cowshed",
"impossible-13",
"impossible-monster-truck-race",
"impossible-tic-tac-toe",
"infinite-soccer",
"infinity-loop",
"into-the-pit",
"iron-snout",
"its-story-time",
"izowave-build-and-defend",
"james-gun",
"jamming-car-escape",
"jelly-truck",
"jet-boy",
"jetpack-joyride",
"jigsaw-photo-puzzle-christmas",
"jigsaw-photo-puzzle-summer",
"jigsaw-photo-puzzle-winter",
"jollyworld",
"jr-chess",
"jump-and-hover",
"jumping-shell",
"jungle-bubble-shooter-mania",
"just-park-it-11",
"just-park-it-12",
"kart-race-3d",
"kart-wars",
"kawaii-dressup",
"kix-dream-soccer",
"kubi-pets",
"la-petite-avril",
"last-warriors",
"lava-bird",
"leader-strike",
"ledge-throw",
"lemonade-stand",
"lethal-sniper-3d-army-soldier",
"level-devil",
"life-choices-life-simulator",
"life-the-game",
"light-the-lamp",
"linebacker-alley-2",
"lines-to-fill",
"little-master-cricket",
"logic-master-1",
"lords-of-gomoku",
"lovely-fox",
"ludo-hero",
"ludo-multiplayer",
"mad-day",
"mad-truck-challenge-special",
"madalin-stunt-cars-2",
"madalin-stunt-cars-3",
"madzoong",
"mafia-wars",
"mahjong-linker-kyodai-game",
"mahjong",
"make-it-meme",
"marble-dash",
"masked-forces",
"master-checkers",
"master-chess",
"math-mahjong-relax",
"maze-path-of-light",
"maze-planet-3d",
"meow-merge",
"merge-cakes",
"merge-cyber-racers",
"merge-party",
"merge-round-racers",
"merge-to-million",
"mermaid-dress-up",
"midnight-drive",
"mimelet",
"minecraft-builder",
"minesweeper",
"mini-guardians-castle-defense",
"mini-train",
"minibattles",
"misland",
"mob-city",
"monkey-mart",
"monster-merge",
"monster-tracks",
"monster-truck-racing-arena",
"monsters-wheels-special",
"moto-maniac-2",
"moto-maniac",
"moto-road-rash-3d",
"moto-space-racing-2-player",
"moto-x3m-2",
"moto-x3m-pool-party",
"moto-x3m-spooky-land",
"moto-x3m-winter",
"moto-x3m",
"moving-truck-bounty",
"mowing-mazes",
"mummys-path-deluxe",
"murder",
"my-pony-my-little-race",
"mystical-birdlink",
"n-gon",
"narrow-one",
"nightpointio",
"noob-drive",
"noob-hook",
"nugget-royale",
"odd-bot-out",
"off-road-rain-cargo-simulator",
"offroader-v5",
"one-button-bounce",
"one-hundred-castles-solitaire",
"onet-fruit-classic",
"onion-boy",
"ooze-odyssey",
"orange",
"orbital-survival",
"ovo-dimensions",
"ovo",
"pack-a-bag",
"paint-strike",
"panda-bubble-shooter",
"panda-simulator-3d",
"papa-louie-when-pizzas-attack",
"papas-cheeseria",
"papas-donuteria",
"papas-hotdoggeria",
"papas-pancakeria",
"papas-pizzeria",
"papas-taco-mia",
"paper-fighter-3d",
"paper-io-2",
"paper-minecraft",
"parking-fury-2",
"parking-fury-3d-beach-city",
"parking-fury-3d-bounty-hunter",
"parking-fury-3d-night-thief",
"parking-fury-3d",
"parking-fury",
"parkour-block-3d",
"parkour-race",
"penalty-kick-online",
"penalty-rivals",
"penalty-shooters-2",
"peppa-pig-basketball",
"perfect-peel",
"photo-puzzle-slide-edition",
"ping-pong-chaos",
"pink",
"pirates-merger",
"pixel-gun-survival",
"pixel-puzzles",
"pixwars-2",
"pizzafall",
"pizzapunk",
"plactions",
"platform-countdown",
"pond-rrace",
"pony-dressup-2",
"pony-run-magic-trails",
"pool-club",
"poor-bunny",
"pop-it-master",
"popit-vs-spinner",
"power-badminton",
"pre-civilization-bronze-age",
"princess-room-cleaning",
"prismo-puzzles",
"puffy-cat-2",
"puffy-cat",
"pull-my-tongue",
"puppet-master",
"rabbit-samurai",
"raft-wars-2",
"raft-wars",
"ragdoll-hit",
"rainbow-star-pinball",
"rally-point-2",
"rally-point-3",
"rally-point-5",
"rally-point",
"real-cars-in-city",
"real-city-driving-2",
"real-simulator-monster-truck",
"rebels-clash",
"recoil",
"red-ball-4",
"refuge-solitaire",
"restaurant-aand-cooking",
"retro-bowl",
"retro-highway",
"rhomb",
"riddle-school",
"rio-rex",
"rocket-pult",
"rocket-soccer-derby",
"rodeo-stampede-savannah",
"rolling-sky",
"rolly-vortex",
"roly-poly-monsters",
"rooftop-snipers-2",
"rooftop-snipers",
"rowdy-city-wrestling",
"rowdy-wrestling",
"rree-kick-screamers",
"run-3-editor",
"runman-race-around-the-world",
"running-fred",
"rush-race-motocross",
"sausage-flip",
"school-bus-demolition-derby",
"scrap-divers",
"scrap-metal",
"shady-bears",
"shape-fold-animals",
"shape-fold-nature",
"shape-fold",
"sharkio",
"shoot-stickman",
"shootz",
"short-life",
"short-ride",
"shortcut-race",
"simply-prop-hunt",
"sink-it",
"skateboard-hero",
"sketchbook-04",
"skibidi-shooter",
"skiing-fred",
"sky-mad",
"slalom-hero",
"slime-laboratory-3",
"slime-laboratory",
"slime-road",
"sling-kong",
"slope-2-multiplayer",
"slope-2",
"slope-3",
"slope-city",
"slope-tunnel",
"slope",
"slopey",
"slot-car-racing",
"snail-bob-6",
"snake-solver",
"snake",
"snakeis-mlg-edition",
"snakes-and-ladders",
"sniper-gun-shooting",
"snow-rider-3d",
"soccar",
"soccer-random",
"soccer-skills-champions-league",
"soccer-skills-euro-cup",
"soccer-skills-world-cup",
"solitaire-reverse",
"solitaire",
"space-major-miner",
"space-thing",
"speed-boat-extreme-racing",
"speed-pool-king",
"spider-solitaire",
"squish-run",
"stack-ball",
"stack-bump-3d",
"stack-city",
"stack",
"stacktris",
"stair-race-3d",
"stealing-the-diamond",
"stick-defenders",
"stick-fighter",
"stick-fortress",
"stick-merge",
"stickman-archero-fight",
"stickman-army-team-battle",
"stickman-army-the-resistance",
"stickman-bike-pr",
"stickman-bike",
"stickman-boxing-ko-champion",
"stickman-bridge-constructor",
"stickman-climb-2",
"stickman-crazy-box",
"stickman-fight-ragdoll",
"stickman-fighter-epic-battle-2",
"stickman-fighter-mega-brawl",
"stickman-golf",
"stickman-hook",
"stickman-parkour-3",
"stickman-ragdoll-crash-fun",
"stickman-run",
"stickman-school-run",
"stock-car-hero",
"street-ball-jam",
"striker-dummies",
"stunt-car-challenge-3",
"stupid-zombies-2",
"stupid-zombies",
"subway-runner",
"subway-surfers-beijing",
"subway-surfers-houston",
"subway-surfers-monaco",
"subway-surfers-newyork",
"subway-surfers-sanfransisco",
"sudoku-calendar",
"sudoku-village",
"sudoku",
"sumo-party",
"super-bike-the-champion",
"super-fowlst-2",
"super-girl-story",
"super-hexbee-merger",
"super-hot",
"super-liquid-soccer",
"super-mario-bros",
"super-mx-the-champion",
"super-racing-gt-drag-pro",
"super-speeder",
"super-star-car",
"super-tunnel-rush",
"superbattle-2",
"superbike-hero",
"superbrawl",
"supernova",
"survival-express",
"survivor-in-rainbow-monster",
"swatforce-vs-terrorists",
"sweety-mahjong",
"swingo",
"tag-2",
"tag",
"tank-ball-monster-battle",
"tank-trouble-2",
"tanuki-sunset",
"teleport-jumper",
"temple-of-boom",
"temple-run-2-frozen-festival",
"temple-run-2-frozen-shadows",
"temple-run-2-holi-festival",
"temple-run-2-jungle-fall",
"temple-run-2",
"tetra-blocks",
"tetris-99",
"tetris-flash",
"the-impossible-quiz",
"the-little-giant",
"there-is-no-game",
"three-goblets",
"throw-it-higher",
"thumb-fighter-christmas",
"thumb-fighter",
"thunderdogs-io",
"tictactoe",
"tiger-simulator-3d",
"time-clones",
"tingly-ppyramid-solitaire",
"tiny-fishing",
"tiny-town-racing",
"toaster-dash",
"tomb-of-the-mask-color",
"tomb-of-the-mask",
"toon-off",
"top-speed-3d",
"top-speed-racing-3d",
"total-recoil",
"touchdowners",
"towntopia",
"traffic-escape",
"traffic-mania",
"traffic-rider",
"traffic-rush",
"tricks",
"trivia-crack",
"truck-loader-2",
"truck-loader-3",
"truck-loader-4",
"truck-loader-5",
"truck-loader",
"true-love-calculator",
"tube-jumpers",
"tumble-wrestling",
"tunnel-rush",
"turbo-moto-racer",
"twice",
"twirl",
"two-ball-3d-dark",
"two-button-bounce",
"two-neon-boxes",
"typing-fighter",
"ultimate-car-driving",
"ultimate-sudoku",
"ultimate-tic-tac-toe",
"unicycle-hero",
"unicycle-legend",
"unpuzzler",
"vectaria-io",
"vex-4",
"vex-5",
"vex-6",
"vex-7",
"vex-8",
"village-craft",
"villain-quinn-my-drawing-portfolio",
"volley-random",
"volleyball-challenge",
"war-of-caribbean-pirates",
"water-color-sort",
"water-polo-ragdoll",
"we-become-what-we-behold",
"we-skate",
"westoon",
"where-is-my-cat",
"who-is",
"wizard-mike",
"wood-blocks-3d",
"word-boss",
"word-city-crossed",
"word-city-uncrossed",
"word-monsters",
"wordle-unlimited",
"wordoku",
"words-search-classic-edition",
"wordy-pop",
"world-conquest",
"worlds-hardest-game-2",
"worlds-hardest-game-3",
"wrassling",
"yarn-untangle",
"yohohoio",
"yokai-dungeon",
"yummy-toast",
"zeepkist-crash-2d",
"zen-blocks",
"zombie-derby-pixel-survival",
"zombie-outbreak-arena",
"zombie-siege-outbreak",
"zoom-be-2",
"zoom-be-3",
"zoom-be",
"zooplop",
"zuma-boom"
]

# Folder containing HTML files
folder_path = r"V:\html"

# Get all HTML files
html_files = [f for f in os.listdir(folder_path) if f.endswith(".html")]

for i, file in enumerate(html_files):
    if i < len(games):
        file_path = os.path.join(folder_path, file)

        # Rename the HTML file using original game name (No capitalization, no space change)
        new_filename = f"{games[i]}.html"
        new_file_path = os.path.join(folder_path, new_filename)
        os.rename(file_path, new_file_path)

        # Update the HTML content
        with open(new_file_path, 'r', encoding='utf-8') as f:
            html_content = f.read()

        # Format game name for content replacement
        formatted_game_name = format_game_name(games[i])

        # Replace '{{text}}' with formatted game name
        html_content = re.sub(r"{{text}}", formatted_game_name, html_content, flags=re.IGNORECASE)

        with open(new_file_path, 'w', encoding='utf-8') as f:
            f.write(html_content)

        print(f"Updated and renamed: {file} -> {new_filename}")

print("✅ All HTML files have been updated and renamed successfully!")
